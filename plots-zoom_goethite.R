...
## all endmembers
goethite_rast_list <- make_allem_rast_zoom(mineral_name = 'Goethite', my_group_name = 'group.1um', my_ext)
goethite_rast <- goethite_rast_list$rast
goethite_rast_list$lyrs_in_img
goethite_rast_list$all_lyrs
n_goethite <- max(as.data.frame(freq(goethite_rast))[['value']])
n_goethite
# set coltab 
coltab_all <- data.frame(values = 1:length(goethite_rast_list$lyrs_in_img),
                         names = goethite_rast_list$lyrs_in_img,
                         cols = scico(length(goethite_rast_list$lyrs_in_img), palette = 'batlow'))

coltab(goethite_rast) <- coltab_all[, c('values', 'cols')]

p3a <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(goethite_rast, axes = FALSE, colNA = 'black')
}

# goethite legend

goethite_pal <- coltab_all[,c('names', 'cols')]

make_goethite_legend <- function(){
  par(mar = c(0,2,0,0), oma = c(0,0,0,0))
  plot.new()
  legend('topleft',
         ncol = 3,
    # x = 0.01, y = 0.85, 
         # cex = 0.5,
         title.adj = 0,
         legend = stringr::str_remove(goethite_pal$names, pattern = stringr::fixed(".fit")), 
         fill = goethite_pal$cols, 
         bty = 'n')
}
...
## mixtures
goethite_mixs_list <- make_mixtures_rast_zoom(mineral_name = 'Goethite', my_group_name = 'group.1um', my_ext)
goethiteMIX_rast <- goethite_mixs_list$rast
goethite_mixs_list$lyrs_in_img
goethite_mixs_list$all_mixs_ems
n_goethiteMIX <- max(as.data.frame(freq(goethiteMIX_rast))[['value']])

coltab_mix <- coltab_all %>% dplyr::filter(names %in% goethite_mixs_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_goethiteMIX)
coltab(goethiteMIX_rast) <- coltab_mix[, c('values', 'cols')]

p3b <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(goethiteMIX_rast, axes = FALSE, colNA = 'black')
}

# set coltab with new vals

## just minerals
goethite_min_list <- make_minerals_rast_zoom(mineral_name = 'Goethite', my_group_name = 'group.1um', my_ext)
goethiteMIN_rast <- goethite_min_list$rast
goethite_min_list$lyrs_in_img
n_goethiteMIN <- max(as.data.frame(freq(goethiteMIN_rast))[['value']])

coltab_min <- coltab_all %>% dplyr::filter(names %in% goethite_min_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_goethiteMIN)
coltab(goethiteMIN_rast) <- coltab_min[, c('values', 'cols')]

# plot(goethiteMIN_rast, axes = FALSE, colNA = 'black')
...
## just goethite
png('Fig4_3_goethite.png', width = 900, height = 400)
aa <- cowplot::plot_grid(NULL, p3c, NULL, p3b, NULL, p3a,
                   ...
                   nrow = 1)
cowplot::plot_grid(aa, NULL, make_goethite_legend, nrow = 3, rel_heights = c(1, -0.01, 0.2))
dev.off()
