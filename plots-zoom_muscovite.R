...
## all endmembers
muscovite_rast_list <- make_allem_rast_zoom(mineral_name = 'Muscovite', my_group_name = 'group.2um', my_ext)
muscovite_rast <- muscovite_rast_list$rast
muscovite_rast_list$lyrs_in_img
muscovite_rast_list$all_lyrs
n_muscovite <- max(as.data.frame(freq(muscovite_rast))[['value']])
n_muscovite
# set coltab 
coltab_all <- data.frame(values = 1:length(muscovite_rast_list$lyrs_in_img),
                         names = muscovite_rast_list$lyrs_in_img,
                         cols = scico(length(muscovite_rast_list$lyrs_in_img), palette = 'batlow'))

coltab(muscovite_rast) <- coltab_all[, c('values', 'cols')]

p3a <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(muscovite_rast, axes = FALSE, colNA = 'black')
}

# muscovite legend

muscovite_pal <- coltab_all[,c('names', 'cols')]

make_muscovite_legend <- function(){
  ...
  legend('center',
    ...
    # x = 0.01, y = 0.85, 
         # cex = 0.5,
         title.adj = 0,
         legend = stringr::str_remove(muscovite_pal$names, pattern = stringr::fixed(".fit")), 
         fill = muscovite_pal$cols, 
         bty = 'n')
}
...
## mixtures
muscovite_mixs_list <- make_mixtures_rast_zoom(mineral_name = 'Muscovite', my_group_name = 'group.2um', my_ext)
muscoviteMIX_rast <- muscovite_mixs_list$rast
muscovite_mixs_list$lyrs_in_img
muscovite_mixs_list$all_mixs_ems
n_muscoviteMIX <- max(as.data.frame(freq(muscoviteMIX_rast))[['value']])

coltab_mix <- coltab_all %>% dplyr::filter(names %in% muscovite_mixs_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_muscoviteMIX)
coltab(muscoviteMIX_rast) <- coltab_mix[, c('values', 'cols')]

p3b <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(muscoviteMIX_rast, axes = FALSE, colNA = 'black')
}

# set coltab with new vals

## just minerals
muscovite_min_list <- make_minerals_rast_zoom(mineral_name = 'Muscovite', my_group_name = 'group.2um', my_ext)
muscoviteMIN_rast <- muscovite_min_list$rast
muscovite_min_list$lyrs_in_img
n_muscoviteMIN <- max(as.data.frame(freq(muscoviteMIN_rast))[['value']])

coltab_min <- coltab_all %>% dplyr::filter(names %in% muscovite_min_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_muscoviteMIN)
coltab(muscoviteMIN_rast) <- coltab_min[, c('values', 'cols')]

# plot(muscoviteMIN_rast, axes = FALSE, colNA = 'black')
...
## just muscovite
png('Fig4_3_muscovite.png', width = 900, height = 400)
aa <- cowplot::plot_grid(NULL, p3c, NULL, p3b, NULL, p3a,
                   ...
                   nrow = 1)
cowplot::plot_grid(aa, NULL, make_muscovite_legend, nrow = 3, rel_heights = c(1, -0.01, 0.2))
dev.off()
