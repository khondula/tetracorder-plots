...
## all endmembers
kaolinite_rast_list <- make_allem_rast_zoom(mineral_name = 'Kaolinite', my_group_name = 'group.2um', my_ext)
kaolinite_rast <- kaolinite_rast_list$rast
kaolinite_rast_list$lyrs_in_img
kaolinite_rast_list$all_lyrs
n_kaolinite <- max(as.data.frame(freq(kaolinite_rast))[['value']])
n_kaolinite
# set coltab 
coltab_all <- data.frame(values = 1:length(kaolinite_rast_list$lyrs_in_img),
                         names = kaolinite_rast_list$lyrs_in_img,
                         cols = scico(length(kaolinite_rast_list$lyrs_in_img), palette = 'batlow'))

coltab(kaolinite_rast) <- coltab_all[, c('values', 'cols')]

p3a <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(kaolinite_rast, axes = FALSE, colNA = 'black')
}

# kaolinite legend

kaolinite_pal <- coltab_all[,c('names', 'cols')]

make_kaolinite_legend <- function(){
  ...
  legend('center',
    ...
    # x = 0.01, y = 0.85, 
         # cex = 0.5,
         title.adj = 0,
         legend = stringr::str_remove(kaolinite_pal$names, pattern = stringr::fixed(".fit")), 
         fill = kaolinite_pal$cols, 
         bty = 'n')
}
...
## mixtures
kaolinite_mixs_list <- make_mixtures_rast_zoom(mineral_name = 'Kaolinite', my_group_name = 'group.2um', my_ext)
kaoliniteMIX_rast <- kaolinite_mixs_list$rast
kaolinite_mixs_list$lyrs_in_img
kaolinite_mixs_list$all_mixs_ems
n_kaoliniteMIX <- max(as.data.frame(freq(kaoliniteMIX_rast))[['value']])

coltab_mix <- coltab_all %>% dplyr::filter(names %in% kaolinite_mixs_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_kaoliniteMIX)
coltab(kaoliniteMIX_rast) <- coltab_mix[, c('values', 'cols')]

p3b <- function(){
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
  plot(kaoliniteMIX_rast, axes = FALSE, colNA = 'black')
}

# set coltab with new vals

## just minerals
kaolinite_min_list <- make_minerals_rast_zoom(mineral_name = 'Kaolinite', my_group_name = 'group.2um', my_ext)
kaoliniteMIN_rast <- kaolinite_min_list$rast
kaolinite_min_list$lyrs_in_img
n_kaoliniteMIN <- max(as.data.frame(freq(kaoliniteMIN_rast))[['value']])

coltab_min <- coltab_all %>% dplyr::filter(names %in% kaolinite_min_list$lyrs_in_img) %>%
  dplyr::select(-values) %>% dplyr::mutate(values = 1:n_kaoliniteMIN)
coltab(kaoliniteMIN_rast) <- coltab_min[, c('values', 'cols')]

# plot(kaoliniteMIN_rast, axes = FALSE, colNA = 'black')
...
## just kaolinite
png('Fig4_3_kaolinite.png', width = 900, height = 400)
aa <- cowplot::plot_grid(NULL, p3c, NULL, p3b, NULL, p3a,
                   ...
                   nrow = 1)
cowplot::plot_grid(aa, NULL, make_kaolinite_legend, nrow = 3, rel_heights = c(1, -0.01, 0.2))
dev.off()
